                ...
                db.query(`SELECT temp, temp_feels, pressure, humidity, weatherid, windspeed, winddeg, clouds, rain, UNIX_TIMESTAMP(sunrise), UNIX_TIMESTAMP(sunset), UNIX_TIMESTAMP(timestamp), timestamp FROM weather`, (err, res) => {
                    ...
                    db.query(`SELECT id, origin, UNIX_TIMESTAMP(timestamp) FROM instances WHERE (id=${id})`, (err, res) => {
                        ...
                        form = form
                            ...
                            .replace('#connectionTimestampUNIX#', instances[0]['UNIX_TIMESTAMP(timestamp)'])
                            .replace('#connectionTimestamp#', new Date(instances[0]['UNIX_TIMESTAMP(timestamp)']))
                            /// Sound table
                            // Titre
                            ...
                            // Capteur 1
                            .replace('#value_d_1#', formatLevel(sound1[sound1.length - 1]['level']))
                            .replace('#value_d-1_1#', formatLevel(sound1[sound1.length - 2]['level']))
                            .replace('#value_d-2_1#', formatLevel(sound1[sound1.length - 3]['level']))
                            .replace('#value_d-3_1#', formatLevel(sound1[sound1.length - 4]['level']))
                            .replace('#value_d-4_1#', formatLevel(sound1[sound1.length - 5]['level']))
                            .replace('#value_d-5_1#', formatLevel(sound1[sound1.length - 6]['level']))
                            .replace('#value_d-6_1#', formatLevel(sound1[sound1.length - 7]['level']))
                            // Capteur 2
                            .replace('#value_d_2#', formatLevel(sound2[sound2.length - 1]['level']))
                            .replace('#value_d-1_2#', formatLevel(sound2[sound2.length - 2]['level']))
                            .replace('#value_d-2_2#', formatLevel(sound2[sound2.length - 3]['level']))
                            .replace('#value_d-3_2#', formatLevel(sound2[sound2.length - 4]['level']))
                            .replace('#value_d-4_2#', formatLevel(sound2[sound2.length - 5]['level']))
                            .replace('#value_d-5_2#', formatLevel(sound2[sound2.length - 6]['level']))
                            .replace('#value_d-6_2#', formatLevel(sound2[sound2.length - 7]['level']))
                            // Capteur 3
                            .replace('#value_d_3#', formatLevel(sound3[sound3.length - 1]['level']))
                            .replace('#value_d-1_3#', formatLevel(sound3[sound3.length - 2]['level']))
                            .replace('#value_d-2_3#', formatLevel(sound3[sound3.length - 3]['level']))
                            .replace('#value_d-3_3#', formatLevel(sound3[sound3.length - 4]['level']))
                            .replace('#value_d-4_3#', formatLevel(sound3[sound3.length - 5]['level']))
                            .replace('#value_d-5_3#', formatLevel(sound3[sound3.length - 6]['level']))
                            .replace('#value_d-6_3#', formatLevel(sound3[sound3.length - 7]['level']))
                            /// Weather table
                            ...
                            // Wind
                            .replace('#value_d_wind#', formatWind(weather[weather.length - 1]['windspeed']) + ' m/s')
                            .replace('#value_d-1_wind#', formatWind(weather[weather.length - 2]['windspeed']) + ' m/s')
                            .replace('#value_d-2_wind#', formatWind(weather[weather.length - 3]['windspeed']) + ' m/s')
                            .replace('#value_d-3_wind#', formatWind(weather[weather.length - 4]['windspeed']) + ' m/s')
                            .replace('#value_d-4_wind#', formatWind(weather[weather.length - 5]['windspeed']) + ' m/s')
                            .replace('#value_d-5_wind#', formatWind(weather[weather.length - 6]['windspeed']) + ' m/s')
                            .replace('#value_d-6_wind#', formatWind(weather[weather.length - 7]['windspeed']) + ' m/s')
                            // Weather condition
...
function formatLevel(level) {
    ...
    let color;
    if (pct > 1) pct = 1;
    if (pct <= 1 && pct > 0.75) color = 'rgb(255,0,0)'
    if (pct <= 0.75 && pct > 0.5) color = 'rgb(255,110,0)'
    if (pct <= 0.5 && pct > 0.25) color = 'rgb(255,232,0)'
    if (pct <= 0.25 && pct > 0) color = 'rgb(0,255,0)'
    return `<div style="text-shadow: 0 0 3px ${color}">${level}</div>`
}

function formatWind(speed) {
    if (speed >= 10.5) {
        return `<div style="text-shadow: 0 0 3px #f00">${speed}</div>`
    } else {
        return speed;
    }
}